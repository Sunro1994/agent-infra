...
- [x] **Step 1: install.sh 작성 (Phase 1 분량만)**
...
- [x] **Step 1: 기존 CLAUDE.md 끝에 sentinel 블록 추가**
...
- [x] **Step 3: Claude Code 재시작 후 정상 로드 확인**
...
- [x] **Step 3: Claude Code 재시작 후 스킬 목록에서 karpathy-guidelines 사라졌는지 확인**
...
- [x] **Step 3: Phase 1 종료 조건 확인 (verifiable)**
...
- [x] **Step 1: session-end-retro.sh 작성**
...
- [x] **Step 1: 현재 settings.json 백업**

```bash
cp ~/.claude/settings.json ~/.claude/_backups/agent-infra-phase2-pre.json
```

- [x] **Step 2: settings.json 수정 — `hooks` 객체에 4개 추가**
...
- [x] **Step 4: Claude Code 재시작 후 hook 동작 확인 (사용자 수동)**
...
- [x] **Step 1: 종료 조건 verification 표 작성**
...
- [x] **Step 1: install.sh 의 symlink 단계 확장**
...
- [x] **Step 3: settings.json 의 `permissions` 영역에 sub-agent 권한 명시**
...
- [x] **Step 2: 정적 HTML 1 페이지로 최소 webapp 생성**
...
- [x] **Step 3: 더미 TASK.md + plan**

`tests/e2e-sandbox/docs/tasks/signup.md`:
```markdown
- [x] [T-001] 이메일 빈 값 입력 시 가입 차단
- [x] [T-002] 비밀번호 8자 미만 입력 시 가입 차단
- [x] [T-003] 정상 입력 시 "OK: <email>" 메시지 표시
```
...
- [x] **Step 4: 로컬 서버 실행 + 즉시 종료 (사용자 메모)**
...
- [x] **Step 2: 메인 Claude 세션에서 qa-agent 호출**
...
- [x] **Step 1: 의도된 트랜잭션 버그가 있는 더미 diff 작성**
...
- [x] **Step 2: 메인 Claude에서 review-agent 호출**
...
- [x] **Step 1: skills 디렉토리 개별 symlink 추가**
...
- [x] **Step 3: Claude Code 재시작 후 스킬 인식 확인**
...
- [x] **Step 1: 가상 feature "signup" 6단계 통과**
...
- [x] **Step 2: 종료 후 새 세션 시작 시 `session-start-retro-alert.sh` 가 회고 DRAFT 알림**
